...
function EduAndPro({ moveForward }) {
  ...
  const [internData2, setInternData2] = useState(
    JSON.parse(localStorage.getItem("internData2")) ?? {
      qualifications: [],
      qualification_types: [],
      qualifications_docs: [],
      passport: "",
  ...
  const [passportPhotographName, setPassportPhotographName] = useState(
    internData2?.passport?.name ?? ""
  );

  useEffect(() => {
    !localStorage.getItem("internData") && navigate("/energy-intern");
  }, []);
  ...
  const handleAddQualificationDocuments = (file, type, name) => {
    ...
    reader.onloadend = () => {
      ...
      setInternData2({
        ...internData2,
        qualifications: [...internData2.qualifications, name],
        qualification_types: [...internData2.qualification_types, type],
        qualifications_docs: [...internData2.qualifications_docs, obj],
      });
  ...
  const handleRemoveQualificationData = (name) => {
    const idx = internData2?.qualifications?.indexOf(name);
    const qualify = internData2?.qualifications.filter(
      (item, elemIndex) => elemIndex !== idx
    );
    const qualify_docs = internData2?.qualifications_docs.filter(
      (item, elemIndex) => elemIndex !== idx
    );
    const qualify_types = internData2?.qualification_types.filter(
      (item, elemIndex) => elemIndex !== idx
    );
    setInternData2({
      ...internData2,
  ...
  return (
    <div>
      <div className={` ${style.heading}`}>
        <h2 className={style.formOneHeading}>EDU & PRO QUALIFICATIONS</h2>
      ...
      <div>
        ...
        <div>
          ...
          <div className="w-[100%] flex flex-col md:flex-row justify-between p-[.5em] rounded-[10px] text-[black] outline-none mt-[1em] mb-[1em] mx-[0] min-h-[60px]  bg-[white] flex items-center gap-4 ">
            <div className="overflow-x-scroll w-[100%] min-h-[0px] overflow-y-hidden flex gap-2 rounded-[10px]">
              {internData2?.qualifications
                .filter(
                  (item, index) =>
                    internData2?.qualification_types[index] === "Educational"
                )
        ...
        <div>
          ...
          <div className="w-[100%] flex flex-col md:flex-row justify-between p-[.5em] rounded-[10px] text-[black] outline-none mt-[1em] mb-[1em] mx-[0] min-h-[60px]  bg-[white] flex items-center gap-4 ">
            <div className="overflow-x-scroll w-[100%] overflow-y-hidden flex gap-2 rounded-[10px] min-h-[0px]">
              {internData2.qualifications
                .filter(
                  (item, index) =>
                    internData2?.qualification_types[index] === "Professional"
                )
                ?.map((qualification, idx) => (
                  <div key={qualification}>
                    <EduKpi
          ...
          {openQualifications === "professional" && (
            <div className="md:flex items-end gap-10">
              ...
              <div>
                <label className={`${style.form4Label}`}>
                  List Of Professional Qualification{" "}
                </label>
                <input
                  type="text"
      ...
      <div className="flex justify-center md:justify-start mt-[2em] md:mt-[3em]">
        ...
        <button
          onClick={() => {
            let resp = checkIfCompleted(internData2);
            if (resp?.status) {
              setMessage(resp?.dataNotCompleted);
            } else {
              localStorage.setItem("internData2", JSON.stringify(internData2));
              moveForward();
            }
          }}
