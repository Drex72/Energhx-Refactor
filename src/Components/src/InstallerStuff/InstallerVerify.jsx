...
import { convertDataUrlToFile, createInstaller } from "../../Forms/useFetch";
import { useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { CircularProgress } from "@mui/material";

const InstallerVerify = ({ moveForward, moveBackward }) => {
  const [message, setMessage] = useState();
  const navigate = useNavigate();
  const data = JSON.parse(localStorage.getItem("installerData"));
  ...
  useEffect(() => {
    getAllDocs();
    !localStorage.getItem("installerData3") && navigate("/installer");
    getDate();
  }, []);
  ...
  const getAllDocs = () => {
    const elem = JSON.parse(
      localStorage.getItem("installerData2")
    )?.qualifications_docs;
    const arr2 = [];
    elem?.map((data) => {
  ...
  const handleSubmitData = async () => {
    if (checkboxes.checkbox1 && checkboxes.checkbox2) {
      const data1 = JSON.parse(localStorage.getItem("installerData"));
      const data2 = JSON.parse(localStorage.getItem("installerData2"));
      const data3 = JSON.parse(localStorage.getItem("installerData3"));
      ...
      setLoading(true);
      let datas = await createInstaller(data4, moveForward);
      setLoading(false);
      setMessage(datas);
    } else {
...
export default InstallerVerify;
